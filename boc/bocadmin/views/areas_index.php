...
<?php include_once 'inc_areas_search_index.php'; ?>
...
<table class="table table-striped table-hover select-list">
	...
	<tbody class="sort-list">
		...
		<tr data-id="<?php echo $v['id'] ?>" data-sort="<?php echo $v['sort_id'] ?>">
			...
			<td><?php echo get_province_for_area($v['parentid']);?></td>
			<td><?php echo get_city_for_area($v['parentid']);?></td>
...
require(['adminer/js/ui'],function(ui){
	var article = {
		url_audit: "<?php echo site_urlc('areas/audit'); ?>"
		,url_flag: "<?php echo site_urlc('areas/flag'); ?>"
		,url_szm_change: "<?php echo site_urlc('areas/szm_change'); ?>"
