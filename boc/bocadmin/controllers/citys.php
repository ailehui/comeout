 ...
 * 信息类
 */
class Citys extends Modules_Controller
{
    ...
    public function search($cid=false,$page=1){
    	...
    	if($this->input->get('provinces')){
    		$where=array_merge($where,array('parentid'=>$this->input->get('provinces',TRUE)));
    	}
    ...
    public function changeCitys(){
    	$vdata=$this->input->post();
    	$province_id=$vdata['data'];
    	$this->output->set_content_type('application/json')->set_output(json_encode(get_citys_list($province_id)));
    }
    public function changeAreas(){
    	$vdata=$this->input->post();
    	$city_id=$vdata['data'];
    	$this->output->set_content_type('application/json')->set_output(json_encode(get_areas_list($city_id)));
    }
}
